...
export default function WysiwygEditor({
  ...
  return (
    <EditorProvider>
      <div
        className={cn(
          "rounded-md border border-input bg-background focus-within:ring-2 focus-within:ring-ring focus-within:ring-offset-2 [&_a]:underline [&_a]:text-primary",
          className,
        )}
      >
  ...
  ({ $selection }) => {
    if ($selection?.nodeName === "A") {
      ...
      document.execCommand(
        ...
        false,
        Uri
          ? `<a href="${Uri}" target="_blank">${Selection ? Selection : Uri}</a>`
          : Selection,
      );
    }
  },
